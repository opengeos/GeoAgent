...
class MockMap:
    ...
    def add_geojson(self, data, layer_name=None, style=None):
        self.layers.append({"type": "geojson", "data": data, "name": layer_name, "style": style})
...
class VizAgent:
    ...
    def _determine_viz_type(
        ...
        if any(term in plan.intent.lower() for term in ["time series", "temporal", "trend"]):
            return "time_series"
                ...
                if asset_key and asset_key in item['assets']:
                    ...
                    try:
                        if 'viz' in self.tools:
                            ...
                            viz_tool.add_raster_layer(m, asset_url, layer_name, plan.intent)
                        else:
                            # Fallback: use leafmap directly
                            m.add_raster(asset_url, layer_name=layer_name, fit_bounds=True)
            ...
            if 'viz' in self.tools:
                ...
                if len(data.items) >= 2:
                    ...
                    if 'assets' in first_item and 'assets' in last_item:
                        asset_key = self._select_best_asset(first_item['assets'], plan.intent)
                        if asset_key:
                            first_url = first_item['assets'][asset_key]['href']
                            last_url = last_item['assets'][asset_key]['href']
                            
                            m.add_raster(first_url, layer_name="Time Series Start", fit_bounds=True)
                            m.add_raster(last_url, layer_name="Time Series End", fit_bounds=False)
            ...
            try:
                if data.data_type == "raster":
                    ...
                    if 'assets' in item:
                        ...
                        if asset_key and asset_key in item['assets']:
                            asset_url = item['assets'][asset_key]['href']
                            m.add_raster(asset_url, layer_name="Data Layer", fit_bounds=True)
    ...
    def _select_best_asset(self, assets: Dict[str, Any], intent: str) -> Optional[str]:
        ...
        preference_order = [
            "visual", "rgb", "red", "nir", "B04", "B03", "B02", "B08",
            "swir", "B11", "B12"
        ]
