...
class PlannerOutput(BaseModel):
    ...
    intent: str = Field(description="The analysis intent (e.g., 'compute_ndvi', 'find_deforestation')")
    ...
    time_range: Optional[Dict[str, Union[str, datetime]]] = Field(
        default=None,
        description="Temporal range with start_date and end_date"
    )
...
class DataResult(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="Additional metadata about the data search"
    )
    data_type: str = Field(
...
class GeoAgentResponse(BaseModel):
    ...
    plan: PlannerOutput = Field(
        description="The parsed query plan"
    )
    data: Optional[DataResult] = Field(
        default=None,
        description="Data retrieval results"
    )
    analysis: Optional[AnalysisResult] = Field(
        default=None,
        description="Analysis results if analysis was performed"
    )
    map: Optional[Any] = Field(  # leafmap.Map - using Any to avoid import issues
        default=None,
        description="Generated leafmap visualization"
    )
    code: str = Field(
        default="",
        description="All generated Python code from the pipeline"
    )
    success: bool = Field(
        default=True,
        description="Whether the overall pipeline succeeded"
    )
    error_message: Optional[str] = Field(
        default=None,
