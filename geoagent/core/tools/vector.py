...
try:
    ...
    gpd = pd = Point = Polygon = LineString = MultiPolygon = box = unary_union = fiona = None
...
def read_vector(
    ...
    try:
        ...
        for col in gdf.columns:
            if col == gdf.geometry.name:
                col_info = {
                    "name": col,
                    "type": "geometry",
                    "geometry_type": gdf.geometry.type.mode().iloc[0] if not gdf.empty else "unknown"
                }
            else:
                col_info = {
                    "name": col,
                    ...
                    "unique_count": gdf[col].nunique() if gdf[col].dtype == 'object' else None
                }
            ...
            for idx, row in sample_df.iterrows():
                ...
                for col in sample_df.columns:
                    if col == gdf.geometry.name:
                        ...
                        geom = row[col]
                        if geom is not None:
                            sample_row[col] = f"{geom.geom_type} (bounds: {list(geom.bounds)})"
                        else:
                            sample_row[col] = None
                    else:
        ...
        logger.info(f"Successfully read vector data: {len(gdf)} features from {path_or_url}")
    ...
    try:
        ...
        if how in ['inner', 'left']:
            ...
            match_stats = {
                ...
                "features_without_matches": left_count - len(matches_per_feature) if how == 'left' else None,
                "max_matches_per_feature": int(matches_per_feature.max()) if not matches_per_feature.empty else 0,
                "mean_matches_per_feature": float(matches_per_feature.mean()) if not matches_per_feature.empty else 0
            }
        else:
            match_stats = {}
    ...
    try:
        ...
        if analysis_type == "summary":
            ...
            if not valid_mask.all():
                ...
                for idx, geom in invalid_gdf.geometry.items():
                    try:
                        ...
                        validation_errors.append({"index": idx, "error": "Unknown validation error"})
                
                results["validation_errors"] = validation_errors[:10]  # Limit to first 10
            ...
            results = {
                ...
                "simple_geometries": int(gdf.geometry.is_simple.sum()) if hasattr(gdf.geometry, 'is_simple') else None,
                "ring_geometries": int(gdf.geometry.is_ring.sum()) if hasattr(gdf.geometry, 'is_ring') else None
            }
