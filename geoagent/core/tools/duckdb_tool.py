...
def query_spatial_data(
    ...
    """
    if duckdb is None:
        return [{"error": "DuckDB is not installed. Please install with: pip install duckdb"}]
    ...
    try:
        ...
        if data_path:
            ...
            if data_path.endswith(('.parquet', '.pq')):
                ...
                view_sql = f"CREATE VIEW data AS SELECT * FROM read_csv_auto('{data_path}')"
            elif data_path.endswith('.json'):
                view_sql = f"CREATE VIEW data AS SELECT * FROM read_json_auto('{data_path}')"
...
def query_overture(
    ...
    try:
        ...
        if bbox:
            ...
            view_sql = f"CREATE VIEW overture_data AS SELECT * FROM read_parquet('{data_url}')"
