...
def load_raster(
    ...
    if xr is None or rxr is None:
        return {"error": "rioxarray and xarray are required. Install with: pip install rioxarray xarray"}
    ...
    try:
        ...
        dataset_info = {
            "shape": ds.shape,
            "dims": list(ds.dims),
            "coords": {k: {"size": v.size, "dtype": str(v.dtype)} for k, v in ds.coords.items()},
            "attrs": dict(ds.attrs),
            "dtype": str(ds.dtype),
            "nbytes": ds.nbytes
        }
        ...
        data_summary = {
            ...
            "pixel_count": dataset_info["shape"][-2] * dataset_info["shape"][-1] if len(dataset_info["shape"]) >= 2 else 0,
...
def compute_index(
    ...
    index_formulas = {
        ...
        "EVI": ("nir", "red", "blue", lambda nir, red, blue: 2.5 * ((nir - red) / (nir + 6 * red - 7.5 * blue + 1))),
        "NDBI": ("swir1", "nir", lambda swir1, nir: (swir1 - nir) / (swir1 + nir)),
        "SAVI": ("nir", "red", lambda nir, red: 1.5 * (nir - red) / (nir + red + 0.5)),
        "MSAVI": ("nir", "red", lambda nir, red: (2 * nir + 1 - np.sqrt((2 * nir + 1)**2 - 8 * (nir - red))) / 2),
    ...
    try:
        ...
        if len(band_arrays) == 2:
            ...
            return {"error": f"Unexpected number of bands required: {len(band_arrays)}", "success": False}
        ...
        spatial_ref = {
            ...
            "transform": list(index_result.rio.transform()) if index_result.rio.transform() else None,
        ...
        return {
            ...
            "bands_used": {name: band_mapping[name] for name in required_bands if name in band_mapping},
            "success": True
        }
...
def zonal_stats(
    ...
    if not all([xr, rxr, gpd, rasterio]):
        return [{"error": "Missing required packages: xarray, rioxarray, geopandas, rasterio"}]
    ...
    try:
        ...
        for idx, zone in zones.iterrows():
            try:
                ...
                values = values[values != src.nodata] if src.nodata is not None else values
