...
try:
    ...
    logger.warning("LangGraph not available. GeoAgent will use simple sequential execution.")
...
class GeoAgent:
    ...
    def _sequential_execution(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if state["should_analyze"] and state["data"] and state["data"].total_items > 0:
    ...
    def _plan_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            analysis_keywords = [
                "calculate", "compute", "analyze", "ndvi", "evi", "index",
                "statistics", "mean", "median", "change", "trend", "zonal"
            ]
            state["should_analyze"] = any(kw in intent_lower for kw in analysis_keywords)
            
            # Visualization is usually desired unless explicitly asking for just data
            viz_skip_keywords = ["download", "list", "count", "metadata"]
            state["should_visualize"] = not any(kw in intent_lower for kw in viz_skip_keywords)
            
            logger.debug(f"Plan created: analyze={state['should_analyze']}, visualize={state['should_visualize']}")
    ...
    def _fetch_data_node(self, state: AgentState) -> AgentState:
        ...
        try:
            if state["plan"]:
                ...
                logger.debug(f"Data fetched: {data.total_items} items of type {data.data_type}")
            else:
    ...
    def _should_analyze(self, state: AgentState) -> bool:
        ...
        """
        return (
            state["should_analyze"] and 
            state["data"] is not None and 
            state["data"].total_items > 0 and
            state["error"] is None
        )
