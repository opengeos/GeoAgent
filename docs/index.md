...
tool metadata, optional package adapters, model-provider configuration, and
human-in-the-loop confirmation hooks.

The goal is simple: downstream geospatial packages should not each need to
invent a separate agent framework. GeoAgent provides the shared foundation,
